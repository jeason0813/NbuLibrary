define('mods/vm.bibllistqueries',
    ...
    function (ds, usercontext, dc, tp, presenter, msgr, entitygrid, ep, gf) {
        ...
        function bind() {
            if (!_bind) {
                var query = ds.search(EntityConsts.BibliographicListQuery);
                ...
                grid.bind(query, usercontext.isCustomer() ? 'BiblList_Customer_BibliographicListQuery_Grid' : 'BiblList_Librarian_BibliographicListQuery_Grid');
                _bind = true;
            }
            else
                grid.reload();
        ...
        function cancel(query) {
            ...
            ask.yes(function () {
                var update = ds.update(EntityConsts.BibliographicListQuery, query.Id());
        ...
        function sendNotification(id, queryUpdate) {
            ...
            var getData = ds.get(EntityConsts.BibliographicListQuery, id);
            ...
            $.when(page.loadDefaults()).done(function () {
                $.when(loadActualData).done(function (d) {
                    ...
                    dc.addRelation(EntityConsts.BibliographicListQuery, 'Notification', d.Id);
        ...
        function process(query) {
            ...
            function processInternal() {
                var page = new ep.Page();
                page.bind(EntityConsts.BibliographicListQuery, 'BiblList_BibliographicListQuery_Process', query.Id());
                page.template = 'GeneralForm';
                $.when(page.load()).done(function () {
                    dc.bind(EntityConsts.BibliographicListQuery, page.Entity, query.Id());
        ...
        function editPayment(query) {
            ...
            $.when(job).done(function () {
                ...
                popup.ok(tp.get('btn_create_payment'), function () {
                    dc.addRelation(EntityConsts.BibliographicListQuery, 'Payment', query.Id());
        ...
        return {
            bind: bind,
            template: 'BiblList_BibliographicListQuery_Grid',
            Items: grid.Items,
